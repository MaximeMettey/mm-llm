...
def generate_text_improved(model, tokenizer, prompt, max_length=50, temperature=0.6, top_k=10):
    ...
    with torch.no_grad():
        for _ in range(max_length):
            ...
            # Top-k sampling to reduce repetition
            if top_k > 0:
            ...
            # Stop if we generate a period (basic stopping condition)
...
prompts = ["The quick", "The dog", "Trees"]

print("Improved generation with top-k sampling and lower temperature:\n")
for prompt in prompts:
    print(f"Prompt: '{prompt}'")
    generated = generate_text_improved(model, tokenizer, prompt, max_length=30, temperature=0.5, top_k=5)
